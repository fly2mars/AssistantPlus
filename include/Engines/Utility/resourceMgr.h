...
// Class suResourceMgr is modified by  Philip Romanik, Amy Muntz's code
...
// Method 2:
//
// The user can embed suStringResource objects directly in the code.
...
//Example:
//suStringResource r1(L"0.92", L"参数1", 1);
//suStringResource r2(L"0.18", L"参数2" ,2);
//suStringResource r3(L"1", L"参数3" , 3);
//suResourceMgr::gOnly().exportStrings("r:\\test.xml");
//suResourceMgr::gOnly().importStrings("r:\\test.xml");
//std::wstring p1 = suResourceMgr::gOnly().fetchString(1);
//std::wstring p2 = suResourceMgr::gOnly().fetchString(2);
//std::wstring p3 = suResourceMgr::gOnly().fetchString(3);
//
//
//std::cout << SU::WstringToDouble(p1) << std::endl;
//std::cout << SU::WstringToDouble(p2) << std::endl;
//std::cout << SU::WstringToDouble(p3) << std::endl;
...
// **********************
//
// An suStringResource allows a string to be embedded in an
// object and later replaced with an alternate string.

class suStringResource
{
public:
  static std::string  sNullString;
  static std::wstring sNullWString;

  suStringResource (const std::wstring& str,
...
private:
  ...
  suStringResource (const suStringResource&) {}
  suStringResource& operator= (const suStringResource&) { return *this;}
...
typedef std::map<unsigned int /*id*/, suStringData /*data*/>    stringmap;

class suResourceMgr
{
public:
  static suResourceMgr& gOnly ();  // Singleton object
  ...
  std::wstring fetchStringByNote (std::wstring note);
  // We can use note as the parameter's name
  // Given an name, fetch the value in a string format.
  // Yao added on 2013/4/23

private:
  static suResourceMgr* sOnly_;
  ...
  void parseElement (const std::wstring& data, suWideTools::eBOM endian);
