...
suXMLTools::suXMLTools () {}


suXMLTools::suXMLTools (const std::wstring& xml,
                        suWideTools::eBOM endian)
  : xml_ (xml), endian_ (endian)
{
  // Swap the endian-ness of our string if necessary
  // This is done in place!!
  if (endian_ != suWideTools::gOnly().endian())
    suWideTools::gOnly().swap (xml_);
...
std::wstring suXMLTools::parseNextTag ()
{
...
std::wstring suXMLTools::parseNextEndTag (std::wstring tag)
{
...
std::wstring suXMLTools::parseTag (const std::wstring& start,
...
suXMLElement suXMLTools::parse ()
{
  suXMLElement element;
...
void suXMLTools::writeResource (unsigned int id,
                                const std::wstring& str,
                                const std::wstring& notes)
{
  if (xml_.empty()) {
    // Write the BOM
    xml_.push_back (suWideTools::gOnly().bom());
