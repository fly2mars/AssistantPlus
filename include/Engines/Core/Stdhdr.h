...
namespace Math
{
	...
	const float MINNUM = 0.00000001f;
	...
	__forceinline bool floatEqual(float fa, float fb, float minNum = MINNUM)
	{
		return fabs(fa - fb) < MINNUM;
	}
...
namespace Utility
{
    ...
    void GetUnicodeFileLines(std::ifstream &File, suVector<suUnicodeString> &Output);
    void GetUnicodeFileLines(const suString &Filename, suVector<suUnicodeString> &Output, UINT LineLimit = 0);
	...
	int RunCommand(const suString &ExecutablePath, const suString &CommandLine, bool Blocking);
